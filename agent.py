...
Master Plan: Sections 1-16 | ~4,200 lines | Dependencies: See §10.1
...
CONFIG = {
    ...
    "DB_PATH": None,  # Set in init
    "NOTES_PATH": None,
    "LOG_PATH": None,
    
    # Behavior
    "TRUST_MODE": os.getenv("PINCER_TRUST", "default"),  # plan/default/acceptEdits/auto/dontAsk
...
class AgentState(Enum):
    """Agent state machine for UI feedback and behavior control."""
    IDLE = auto()
    INITIALIZING = auto()
    THINKING = auto()
    PLANNING = auto()
...
class TrustMode(Enum):
    """Graduated trust spectrum for permission system."""
    PLAN = "plan"           # Approve entire plan before execution
    DEFAULT = "default"     # Ask for shell, auto-allow reads
    ACCEPT_EDITS = "acceptEdits"  # Auto-allow edits, ask for shell
    AUTO = "auto"           # ML classifier approves safe commands
    DONT_ASK = "dontAsk"    # Skip most prompts (dangerous still ask)

# Mascot state mapping: (icon, color, label, animation)
MASCOT_STATE = {
...
@dataclass
class ContextBudget:
    """Token budget tracking with automatic alerts."""
...
# ==============================================================================
class PincerLogger:
    """Lightweight logger with file rotation and console output."""
    ...
    def _write(self, level: str, msg: str):
        if {"DEBUG", "INFO", "WARNING", "ERROR"}.get(level, "INFO") >= self.level:
...
"""
    ...
    def _check_model(self) -> bool:
        """Ensure the target model is available or pull it."""
        model = CONFIG["MODEL"]
        ...
        try:
            ...
            if CONFIG["AUTO_PULL"]:
                print_formatted_text(f"  {COLORS['WARNING']}⏳ Pulling {model}... (this may take a few minutes){COLORS['TEXT']}")
                try:
...
# ==============================================================================
class PincerDB:
    """SQLite-based hybrid memory system (episodic + semantic + procedural)."""
    ...
    CREATE TABLE IF NOT EXISTS messages (
        ...
        tool_call_id TEXT,
        metadata TEXT  -- JSON: {thinking: bool, tool: str, ...}
    );
    CREATE INDEX IF NOT EXISTS idx_messages_session ON messages(session_id, ts);
    CREATE INDEX IF NOT EXISTS idx_messages_role ON messages(role);
    
    -- Semantic: Self-notes with categories for RAG
    CREATE TABLE IF NOT EXISTS notes (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        ts REAL NOT NULL,
        category TEXT NOT NULL,  -- observation/error/success/preference/pattern/fact
        content TEXT NOT NULL,
        task_id TEXT,
        embedding BLOB,  -- For sqlite-vec (optional)
        relevance REAL DEFAULT 1.0
    );
    CREATE INDEX IF NOT EXISTS idx_notes_category ON notes(category);
    CREATE INDEX IF NOT EXISTS idx_notes_task ON notes(task_id);
    
    -- Procedural: User preferences & project conventions
    CREATE TABLE IF NOT EXISTS preferences (
        key TEXT PRIMARY KEY,
        value TEXT NOT NULL,
        ts REAL NOT NULL,
        scope TEXT DEFAULT 'global'  -- global/project/file
    );
    
    -- Working: Checkpoints for resume/rollback
    CREATE TABLE IF NOT EXISTS checkpoints (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        ts REAL NOT NULL,
        task_id TEXT NOT NULL,
        plan TEXT,  -- JSON: steps array
        context_summary TEXT,
        status TEXT DEFAULT 'active',  -- active/completed/rolled_back
        snapshot_path TEXT  -- Optional: file snapshot for rollback
    );
    
    -- Tool execution log for auditing
    ...
    def get_messages_for_compaction(self, session_id: str = "default") -> List[Dict]:
        """Get messages eligible for compaction (older, non-essential)."""
    ...
    def write_note(self, category: str, content: str, task_id: str = None, 
        ...
        """Write a self-note for future reference."""
    ...
    def query_notes(self, query: str, categories: List[str] = None, 
        ...
        """Semantic search across notes (basic FTS fallback; sqlite-vec optional)."""
    ...
    def compact_notes(self, max_age_days: int = 7) -> int:
        """Summarize old notes into patterns, archive the rest."""
        ...
        if not old:
            return 0
        # Simple compaction: group by category, concatenate
        summary = {}
        for row in old:
            cat = row["category"]
    ...
    def save_checkpoint(self, task_id: str, plan: List[Dict], context_summary: str,
        ...
        """Save execution checkpoint for resume/rollback."""
        cursor = self.conn.execute(
    ...
    def get_active_checkpoint(self, task_id: str) -> Optional[Dict]:
        """Retrieve active checkpoint for a task."""
        row = self.conn.execute(
    ...
    def log_tool_execution(self, tool_name: str, arguments: Dict, result: Dict,
        ...
        """Log tool execution for auditing and learning."""
        self.conn.execute(
...
# ==============================================================================
class OllamaBackend:
    """Robust Ollama client with streaming, tool calling, and health checks."""
    ...
    async def chat(self, messages: List[Dict], tools: List[Dict] = None, 
        ...
        """
        Chat with Ollama with native tool calling support.
        
        Args:
            messages: List of {role, content} dicts
            tools: Ollama-compatible tool schemas
            stream: If True, return async queue of chunks
            options: Model options (num_ctx, temperature, etc.)
            think: Enable Qwen3 thinking mode
        
        Returns:
            Dict with full response OR asyncio.Queue for streaming
        """
        kwargs = {
            "model": self.model,
            "messages": messages,
...
class WebTools:
    """Three-tier web access: fetch, search, scrape."""
    ...
    async def _get_session(self):
        """Get or create httpx async session."""
    ...
    async def fetch(self, url: str, max_length: int = 5000, 
        ...
        """Fast static fetch with HTML-to-text conversion."""
        ...
        async with self._get_session() as session:
            try:
                ...
                response = await session.get(
                    ...
                    params={"q": query, "kl": "wt-wt"},  # Worldwide
                    headers={"User-Agent": self.USER_AGENT}
                )
                response.raise_for_status()
    ...
    async def scrape(self, url: str, wait_for: str = None, 
        ...
        Fallback to fetch if Playwright unavailable.
        """
        if not self.allow_network:
            raise PermissionError("Network access disabled")
...
# ==============================================================================
class Guardrails:
    """Deny-first rule engine with graduated trust."""
    
    # High-risk patterns that are ALWAYS denied
    DENY_PATTERNS = [
        r"^rm\s+-[a-zA-Z]*f\s+/",           # rm -rf /
        ...
        r"^:(){:|:&};:",                     # Fork bomb variant
        r"^\.\s*\./",                        # Source local script dangerously
    ]
    
    # Safe patterns that never require approval in default+ modes
    SAFE_PATTERNS = [
        ...
        r"^pwd$",                           # Print cwd
        r"^echo\s+.*",                      # Echo
        r"^python[3]?\s+.*\.py$",          # Run Python scripts
        r"^pytest\s+.*",                    # Run tests
        r"^python\s+-m\s+pytest\s+.*",     # Run tests via module
        r"^find\s+.*-type\s+f\s+.*",       # Find files
        r"^grep\s+.*",                      # Search text
        r"^head\s+.*", r"^tail\s+.*",      # View file snippets
    ]
    ...
    def check(self, command: str, tool_name: str = None) -> Tuple[bool, str]:
        """
        Check if a command/tool execution is allowed.
        ...
        # Check deny patterns (ALWAYS enforced)
    ...
    def should_ask_permission(self, command: str, tool_name: str) -> bool:
        """
        Determine if user permission is required based on trust mode.
        """
        # Auto-approve in auto/dontAsk modes (except deny patterns, already checked)
        if self.trust_mode in [TrustMode.AUTO, TrustMode.DONT_ASK]:
            return False
        
        # Plan mode: always ask for execution
        if self.trust_mode == TrustMode.PLAN:
            return True
        
        # Default mode: ask for shell, auto-allow reads
        if self.trust_mode == TrustMode.DEFAULT:
            if tool_name == "execute_command":
                # Check if it's a safe pattern
                return not any(re.search(p, command) for p in self.SAFE_PATTERNS)
            return False  # Auto-allow file ops in default
        
        # AcceptEdits mode: auto-allow file edits, ask for shell
        if self.trust_mode == TrustMode.ACCEPT_EDITS:
            return tool_name == "execute_command"
        
        return True  # Fallback: ask
    
    def record_progress(self):
        """Mark that progress was made (resets no-progress timer)."""
        self.last_progress = time.time()
...
# ==============================================================================
class ToolRegistry:
    """Dynamic tool system with Ollama-compatible schemas."""
    ...
    def _build_schemas(self) -> List[Dict]:
        """Build Ollama-compatible tool schemas."""
        return [
            {
                "type": "function",
                "function": {
                    "name": "read_file",
                    "description": "Read a file with line numbers. Use for understanding code or config.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "path": {"type": "string", "description": "File path (relative or absolute)"},
                            "limit": {"type": "integer", "description": "Max lines to read (default: 100)", "default": 100},
            ...
            },
            {
                ...
                "function": {
                    "name": "write_file",
                    "description": "Create or overwrite a file. Use for generating code or configs.",
                    "parameters": {
                        "type": "object",
                        "properties": {
            ...
            },
            {
                ...
                "function": {
                    "name": "edit_file",
                    "description": "Exact string replacement in a file. Must be unique match.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "path": {"type": "string", "description": "File path"},
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Run a shell command with sandbox protection.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            ...
                            "max_length": {"type": "integer", "description": "Max chars to return", "default": 3000}
                        },
                        "required": ["url"]
                    }
                }
            },
            {
                ...
                "function": {
                    "name": "think",
                    "description": "Record step-by-step reasoning (visible to user).",
                    "parameters": {
                        "type": "object",
                        "properties": {
    ...
    async def execute(self, tool_call: Dict, trust_mode: TrustMode, 
                     cwd: str = ".") -> Dict:
        ...
        try:
            ...
            self.mem.log_tool_execution(name, args, result, 
                                       (time.time()-start)*1000, False, error_msg)
            return result
    ...
    def _build_sandbox_profile(self, cwd: str, allow_network: bool) -> str:
        """Generate sandbox-exec profile for macOS."""
    ...
    async def _think(self, args: Dict, cwd: str) -> Dict:
        """Record reasoning (no-op, just for UI display)."""
        return {"status": "reasoning_recorded", "content": args.get("reasoning", "")}
    
    async def _ask_user(self, args: Dict, cwd: str) -> Dict:
        """Ask clarifying question (stub; real impl would pause for input)."""
        question = args["question"]
        options = args.get("options")
        
        # In real implementation, this would:
        # 1. Display question to user via UI
        # 2. Wait for response
        # 3. Return response to agent loop
        ...
        return {
            ...
            "note": "In production, this pauses the agent loop for user input",
        }
...
# ==============================================================================
class AgentCore:
    """ReAct loop with native tool calling, context management, and compaction."""
...
- Remember: You run locally on the user's machine. No cloud, no data leaving."""
    ...
    def __init__(self, ui_queue: asyncio.Queue, logger: PincerLogger):
        self.ui = ui_queue  # Queue for UI updates
        self.logger = logger
    ...
    def _notify_ui(self, content: str, state: AgentState = None, 
                  metadata: Dict = None):
        """Send update to UI via queue."""
        if state:
            self.state = state
        self.ui.put_nowait({
            "type": "agent_update",
    ...
    def _assemble_context(self, user_input: str) -> List[Dict]:
        """Build message list for LLM with budget awareness."""
        messages = [{"role": "system", "content": self.SYSTEM_PROMPT}]
        self.ctx.system = self._estimate_tokens(self.SYSTEM_PROMPT)
        
        # Add recent notes (semantic memory)
        ...
        # Add recent conversation (episodic memory)
        ...
        # Add tool schemas (overhead)
        self.ctx.tools = self._estimate_tokens(
            json.dumps(self.tools.schemas, indent=2)
        )
    ...
    def _compact_context(self, messages: List[Dict]) -> List[Dict]:
        ...
        1. Budget Reduction: Trim tool output
        2. Snip: Remove oldest non-essential
        3. Microcompact: Summarize related groups
        4. Context Collapse: Replace full reads with summaries
        5. Auto-compact: LLM summarizes conversation
        """
        ...
        # Layer 2: Keep system + recent, drop old
        system_msgs = [m for m in messages if m["role"] == "system"]
        recent_msgs = messages[-15:]  # Keep last 15
        messages = system_msgs + recent_msgs
        
        # Layer 3: Summarize old conversation into note
        old_msgs = [m for m in messages if m["role"] in ["user", "assistant"] 
                   and m not in recent_msgs]
        if old_msgs:
            summary = " | ".join([m["content"][:30] for m in old_msgs[-5:]])
            self.db.write_note("summary", f"Auto-compact: {summary}")
        
        # Recalculate budget
        self.ctx.conversation = sum(
        ...
        self._notify_ui(f"✅ Context compacted ({self.ctx.used}/{self.ctx.max_tokens})", 
                       AgentState.IDLE)
        
        return messages
    
    async def run_loop(self, user_input: str):
        """Main ReAct loop with tool calling."""
        self.turn = 0
        ...
        while self.turn < CONFIG["MAX_TURNS"]:
            ...
            try:
                ...
                if content and CONFIG["SHOW_THINKING"]:
                    self._notify_ui(content[:200] + ("..." if len(content) > 200 else ""), 
                                   AgentState.PLANNING if tool_calls else AgentState.EXECUTING)
                ...
                self.db.add_message("assistant", content, 
                                  self._estimate_tokens(content), self.session_id)
                ...
                # Try to recover: compact and retry once
        ...
        self.db.save_checkpoint(
            self.task_id,
            plan=self.plan,
            context_summary=f"Turns: {self.turn}, Tokens: {self.ctx.used}",
            status="completed"
        )
    ...
    async def run_autonomous(self, goal: str, auto_approve: bool = False):
        """Autonomous mode: plan and execute multi-step tasks."""
        ...
        plan_response = await self.backend.chat(
            [{"role": "user", "content": plan_prompt}],
            stream=False
        )
        
        # Parse plan (simplified; real impl would use structured output)
        plan_text = plan_response["message"]["content"]
        self.plan = [{"step": i+1, "description": line.strip()} 
                    for i, line in enumerate(plan_text.split("\n")) if line.strip()]
        
        self._notify_ui(f"📋 Plan ({len(self.plan)} steps):\n" + 
                       "\n".join(f"{s['step']}. {s['description']}" for s in self.plan[:5]),
                       AgentState.PLANNING)
        ...
        for step in self.plan:
            ...
            if "shell" in step.get("tool", "") or "command" in step.get("args", {}):
                ...
                if not allowed and not auto_approve:
                    ...
                    # In real impl: pause and ask user
                    break
            ...
            # Self-evaluate every 5 steps
            if step["step"] % 5 == 0:
                self.db.save_checkpoint(
                    self.task_id, self.plan, 
                    f"Progress: {step['step']}/{len(self.plan)}",
                    "active"
                )
...
# ==============================================================================
class PincerUI:
    """Full-screen terminal UI with panels, streaming, and mascot."""
    ...
    def __init__(self, logger: PincerLogger):
        ...
        self.focused_panel = 1  # 0=files, 1=chat, 2=activity
...
Context Budget:
  ...
  Breakdown:
    ...
    def _build_app(self) -> Application:
        ...
        # Header: Mascot + Status + Context
        header = Window(
            content=FormattedTextControl(lambda: self._render_header()),
            height=1,
            style=f"bg:{COLORS['BG']} fg:{COLORS['TEXT']}",
        )
        
        # Left Panel: File Tree
        left_panel = Window(
            content=FormattedTextControl(lambda: self._render_file_tree()),
            width=24,
            style=f"bg:{COLORS['SURFACE']} fg:{COLORS['TEXT']}",
        )
        ...
        # Right Panel: Activity & Plan
        right_panel = Window(
            content=FormattedTextControl(lambda: self._render_activity()),
            width=28,
            style=f"bg:{COLORS['SURFACE']} fg:{COLORS['TEXT']}",
        )
        ...
        input_bar = Window(
            content=BufferControl(
                ...
                focusable=True,
                prompt=HTML(f'<style fg="{COLORS["PRIMARY"]}">🦞</style> '),
            ),
            height=1,
            style=f"bg:{COLORS['SURFACE']} fg:{COLORS['ACCENT']}",
        )
        
        # Footer: Key hints
        footer = Window(
            content=FormattedTextControl(
                lambda: f"[{COLORS['TEXT_MUTED']}]Enter:Send │ C-P:Panels │ C-1/2/3:Focus │ /help:Commands │ 🦞 Blue OS[/{COLORS['TEXT_MUTED']}]"
            ),
            height=1,
            style=f"bg:{COLORS['BG']} fg:{COLORS['TEXT_MUTED']}",
        )
        
        # Main layout
        main_area = VSplit([
            ConditionalContainer(left_panel, filter=lambda: self.panels_visible),
            center_panel,
            ConditionalContainer(right_panel, filter=lambda: self.panels_visible),
        ])
        ...
        return Application(
            ...
            style=Style.from_dict({
                "buffer": f"bg:{COLORS['BG']} fg:{COLORS['TEXT']}",
                "input": f"bg:{COLORS['SURFACE']} fg:{COLORS['ACCENT']}",
            }),
        )
    
    def _render_header(self) -> str:
        """Render the header with mascot and status."""
        ...
        return (
            ...
            f"[bold]{label}[/] │ "
            f"{CONFIG['MODEL']} │ "
            f"ctx:[{COLORS['ACCENT']}]{bar} {self.ctx.used}/{self.ctx.max_tokens}[/] │ "
            f"[{COLORS['SUCCESS']}]🔒[/] {self.core.gr.trust_mode.value}"
        )
    
    def _render_file_tree(self) -> str:
        """Render a simple file tree (stub; real impl uses RepoMap)."""
    ...
    def _render_activity(self) -> str:
        """Render activity panel: plan, stats, guardrails."""
    ...
    async def _ui_update_loop(self):
        """Background loop: poll UI queue and refresh display."""
        while True:
            try:
            ...
            # Refresh display at configured rate
            self.app.invalidate()
